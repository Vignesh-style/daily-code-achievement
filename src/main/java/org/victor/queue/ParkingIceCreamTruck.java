...
/**
 * Scaler problem :: Imagine you're an ice cream truck driver in a beachside town. The beach is divided into several sections, and each section has varying numbers of beachgoers wanting ice cream given by the array of integers A.
...
public class ParkingIceCreamTruck {
    ...
    public ArrayList<Integer> slidingMaximum(final List<Integer> A, int B) {
        ...
        while (b < B) {
            ...
            while (!deque.isEmpty() && deque.getLast() < A.get(b)) {
                deque.removeLast();
            }

            deque.addLast(A.get(b));
            b++;
        }
        ...
        while (j < A.size()) {
            ...
            if (deque.getFirst().equals(A.get(i - 1))) {
                deque.removeFirst();
            }

            while (!deque.isEmpty() && deque.getLast() < A.get(j)) {
                deque.removeLast();
            }
